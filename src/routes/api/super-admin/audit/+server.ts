...
import { requireSuperAdminActor } from '$lib/server/authorization/actor';
import { db } from '$lib/server/db';
import { listPlatformAuditEvents, type AuditListResult } from '$lib/server/audit/query';
...
export const GET: RequestHandler = async ({ url, locals }) => {
	try {
		...
		const guard = requireSuperAdminActor(actor);
		if (!guard.ok) {
		...
		const result = await listPlatformAuditEvents(db, {
			limit,
